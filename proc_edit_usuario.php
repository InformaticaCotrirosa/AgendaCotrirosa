...
$codloc = filter_input(INPUT_POST, 'USU_CODLOC', FILTER_SANITIZE_NUMBER_INT);
$desloc = filter_input(INPUT_POST, 'USU_DESLOC', FILTER_DEFAULT);
$sitloc = filter_input(INPUT_POST, 'USU_SITLOC', FILTER_DEFAULT);
$corloc = filter_input(INPUT_POST, 'USU_COR', FILTER_DEFAULT);
...
if (is_null($aPerUsu)) {
    $_SESSION['msg'] = '<div class="alert alert-danger" role="alert">Erro:O usuário não tem permissão para editar os locais cadastrados!</div>';
    header("Location: index.php");
} else {

    //Edita o local
    $sql = "UPDATE USU_TCADLOC SET USU_CODLOC='$codloc', USU_DESLOC='$desloc', USU_SITLOC='$sitloc', USU_COR='$corloc' WHERE USU_CODLOC='$codloc'";
    //Edita a cor dos eventos ja cadastrados
    $sqlevent = "UPDATE USU_TAGELOC SET USU_COR='$corloc' WHERE USU_CODLOC='$codloc'";

    if ($sql !== "") {
        $stid = $conec->query($sql);
        $stid2 = $conec->query($sqlevent);

        //AQUI FAZER UM ALERT CASO TENHA EDITADO O LOCAL CORRETAMENTE
        $linhafec = oci_num_rows($stid);
        if ($linhafec > 0) {
            $_SESSION['msg'] = '<div class="alert alert-success" role="alert">O local foi editado com sucesso!</div>';
            header("Location: index.php");
        } else {
            $_SESSION['msg'] = '<div class="alert alert-danger" role="alert">Erro: O local não foi editado!</div>';
            header("Location: index.php");
        }
    }
}
